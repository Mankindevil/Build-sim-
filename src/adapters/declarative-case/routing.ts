...
import type { DeclarativeDocument } from "./geometry";

export interface DeclarativeRoutingResult {
  cables: RoutedCable[];
  ports: Port[];
  findings: EngineFinding[];
}

export interface DeclarativeCaseRouting {
  buildRouting(parts: PlacedPart[], plan: WiringPlan, catalog: SkuCatalog): DeclarativeRoutingResult;
}

export function createDeclarativeCaseRouting(
  routing: DeclarativeDocument,
  trayCount: number,
): DeclarativeCaseRouting {

/**
 * Turns an electrical plan into millimetre cable runs over declarative geometry.
 *
...
function instantiate(parts: PlacedPart[]): Map<string, Port> {
  ...
  if (dataDecl && pcb) {
    for (let i = 0; i < trayCount; i++) {
      const offset: [number, number] = [(i - (trayCount - 1) / 2) * dataDecl.offsetPitchMm, dataDecl.offsetV];
