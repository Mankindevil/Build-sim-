...
export async function runAutoEnrichment(candidateId, options = {}) {
  ...
  const registryVersion = options.registry?.version ?? activeOfficialRegistry().version;
  const evidence = { candidateInputHash: inputHash, registryVersion, extractorVersion: candidate.extraction?.adapter, contentHash: candidate.extraction?.contentHash, writeEnabled };
  ...
  const draft = await createDraft(candidateId, {}, { ...options, candidate, expectedHash: inputHash, registryVersion });
