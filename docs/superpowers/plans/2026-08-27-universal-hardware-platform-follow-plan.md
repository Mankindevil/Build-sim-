...
- 数据迁移：`tests/plan-v3-migration-rehearsal.test.ts` 2/2 证明方案 dry-run 零写、精确 manifest、外部加密备份、apply、归档 V2 保留与 rollback。新增 `runtime:migrate-price-v2` 以独立 source manifest 锁定 runtime generation/revision、旧 current 原始字节、受治理观察/采集和 catalog；apply 先创建并验证外部加密备份，再逐字节归档旧快照并仅从有效观察重建 v2，失败恢复旧 generation 内容且并发 revision 变化时零迁移写。真实部署 runtime 仅完成价格迁移 dry-run，尚未 apply。
- 聚焦测试：U12 35 files / 109 tests 通过；U9 13/62、U10 20/46、U11 17/45 通过。
- 全量测试：330 files / 1625 tests；默认沙箱内 1622 项通过，3 项仅因禁止本机监听失败，精确回环复跑 2 files / 11 tests 通过。typecheck、三端 build、1045-file content scan 与 diff-check 通过。
- 浏览器测试：计划列出的 12 个本机页面脚本全部通过；未访问外部网络。
- release canary：`npm run release:canary` 已把 N6 阶段 A 固化为生产权威路径；默认离线回归 11 项通过，包含同一 runtime root 内的活动 claim scope 投影、由锁定 adapter/FactSnapshot 生成且明确分开的“当前盘位需求 / 全背板未来能力”，以及明确的 `preparation_only` 流程：只生成断电清点、量尺和补充资料三个 `prepare` 步骤，不含 bench、firmware、first-power、system-install 或高风险步骤。当前两块未放置 SSD 与缺少 PSU 官方线材根数都会保持 unknown，不生成线材；其余通过项包含局部 requirements、空盘位零假线材，以及推断坐标/公差与 component placement/routing/assembly 三域 blocked。新增 `--source-runtime-root` 发布模式在一致性只读快照内把活动 generation 复制到临时目录，再创建 canary plan/version；源 runtime 的 pointer、plans 和 price snapshot 保持逐字节不变。该模式从同一锁定 price snapshot 重放每个精确规格组：单条只接受 `low` 单点，两家以上只接受 `medium/high` 区间，冲突/过期/历史记录不能冒充当前价。CPU/SSD/旧款 PSU 的完整 official fact closure 与 i5-14500 最大睿频功耗 official fact 两项保持 blocked；当前真实 runtime 的 `prices/latest.json` 仍是缺 immutable snapshot ID 的 legacy archive，发布模式会在评估前明确拒绝，因此中国新货价格 checkbox 仍未勾选，也未从 catalog 属性或旧价格补造。
