...
- 全量测试：329 files / 1621 tests；默认沙箱内 3 项仅因禁止本机监听失败，精确回环复跑 2 files / 11 tests 通过。typecheck、三端 build、1043-file content scan 与 diff-check 通过。
- 浏览器测试：计划列出的 12 个本机页面脚本全部通过；未访问外部网络。
- release canary：`npm run release:canary` 已把 N6 阶段 A 固化为生产权威路径；11 项通过，包含同一 runtime root 内的活动 claim scope 投影、由锁定 adapter/FactSnapshot 生成且明确分开的“当前盘位需求 / 全背板未来能力”，以及明确的 `preparation_only` 流程：只生成断电清点、量尺和补充资料三个 `prepare` 步骤，不含 bench、firmware、first-power、system-install 或高风险步骤。当前两块未放置 SSD 与缺少 PSU 官方线材根数都会保持 unknown，不生成线材；其余通过项包含局部 requirements、空盘位零假线材，以及推断坐标/公差与 component placement/routing/assembly 三域 blocked。CPU/SSD/旧款 PSU 的完整 official fact closure 与 i5-14500 最大睿频功耗 official fact 两项保持 blocked，命令退出码为 2，未从 catalog 属性补造。
