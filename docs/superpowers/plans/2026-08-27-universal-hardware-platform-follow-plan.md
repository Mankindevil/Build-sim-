...
- [x] PSU/背板线束区分“当前盘位需求”和“全背板未来能力”。
- [x] 空盘位不生成假数据线。
- [x] 3D/走线显示推断坐标、公差和 blocked 项。
- [x] 缺内存/散热等关键输入时，总热噪为 partial/blocked，不输出完整温度/噪音，也不允许 `powerReady`。
- [ ] 中国新货价格按低置信单点或市场区间显示。
- [x] procedure 只能生成当前安全的 prepare/measurement/补证步骤，不得生成可执行首次通电完成状态。
...
- 提交：本记录随本次本地 release-gate commit 提交；新增 `415ccdf`（Stage B/跨产品内容寻址旅程门禁）、`ba9fc57`（legacy 价格只作为历史并继续生成完整报告）、`7119172`（方案迁移 dry-run 只读）、`7c6c92f`（不可读 runtime 路径生成完整 Doctor 阻断报告）和 `a08a810`（严格旅程清单 authoring CLI）；远程未推送。
...
- 数据迁移：`tests/plan-v3-migration-rehearsal.test.ts` 3/3 证明方案 dry-run 零写、精确 manifest、只读 root 不触碰 `.locks`、外部加密备份、apply、归档 V2 保留与 rollback。新增 `runtime:migrate-price-v2` 以独立 source manifest 锁定 runtime generation/revision、旧 current 原始字节、受治理观察/采集和 catalog；apply 先创建并验证外部加密备份，再逐字节归档旧快照并仅从有效观察重建 v2，失败恢复旧 generation 内容且并发 revision 变化时零迁移写。真实部署 runtime 仅完成价格迁移 dry-run，尚未 apply。
- 聚焦测试：新增 universal journey/release 3 files / 8 tests、相邻 release/holdout/portable/backup/Doctor 8 files / 26 tests、方案迁移/仓储 2 files / 16 tests、Doctor/production operations 3 files / 25 tests 通过；既有 U12 35/109、U9 13/62、U10 20/46、U11 17/45 继续通过。
- 全量测试：332 files / 1633 tests；默认隔离环境内 330 files / 1630 tests 通过，3 项仅在创建 `127.0.0.1` 测试监听时被环境以 `EPERM` 拒绝，未发现产品断言失败。新增 ownership bootstrap/Doctor/backup/deploy 相邻回归 10 files / 42 tests 通过；typecheck、三端 build、纯本地内容检查与 diff-check 通过；密钥扫描器现可在 Git 子进程不可用时使用不跟随 symlink、遵守根 `.gitignore` 的纯文件系统回退，两种枚举均扫描 1057 个文本文件且 0 findings。本轮遵循不开放本机端口约束，未为这 3 项申请额外权限。
- 浏览器测试：计划列出的 12 个本机页面脚本全部通过；未访问外部网络。
- release canary：`npm run release:canary` 已把 N6 阶段 A 固化为生产权威路径；默认离线回归 11 项通过，包含同一 runtime root 内的活动 claim scope 投影、由锁定 adapter/FactSnapshot 生成且明确分开的“当前盘位需求 / 全背板未来能力”，以及明确的 `preparation_only` 流程：只生成断电清点、量尺和补充资料三个 `prepare` 步骤，不含 bench、firmware、first-power、system-install 或高风险步骤。当前两块未放置 SSD 与缺少 PSU 官方线材根数都会保持 unknown，不生成线材；其余通过项包含局部 requirements、空盘位零假线材，以及推断坐标/公差与 component placement/routing/assembly 三域 blocked。新增 `--source-runtime-root` 发布模式在一致性只读快照内把活动 generation 复制到临时目录，再创建 canary plan/version；源 runtime 的 pointer、plans 和 price snapshot 保持逐字节不变。该模式从同一锁定 price snapshot 重放每个精确规格组：单条只接受 `low` 单点，两家以上只接受 `medium/high` 区间，冲突/过期/历史记录不能冒充当前价。CPU/SSD/旧款 PSU 的完整 official fact closure 与 i5-14500 最大睿频功耗 official fact 两项保持 blocked；当前真实 runtime 的 `prices/latest.json` 仍是缺 immutable snapshot ID 的 legacy archive，发布模式会在临时克隆中改用空的 governed current snapshot 继续列出其余阻断，因此中国新货价格 checkbox 仍未勾选，也未从 catalog 属性或旧价格补造。
...
- 针对上述“不可读所以无法先备份”的唯一引导场景，新增 `runtime:bootstrap-access`：默认 plan 模式零写入，只允许活动 generation 的 `plans/.locks`、旧 `plans/.agent-context-audit` 和旧 `transactions` 三个固定目录，绑定 pointer/generation/device/inode/mode/owner。管理员 apply 前必须把计划与完整递归 rollback manifest 放在 runtime 外的私有目录并回填 exact plan hash；遍历拒绝 symlink、硬链接、特殊文件和跨设备边界，owner 变更使用 `O_NOFOLLOW` descriptor-bound 写，只改 owner、不改 mode/content，中途失败恢复已改元数据；rollback 必须同时提供原计划、manifest 及两者 exact hash。真实 runtime dry-run 精确得到 3 个目标、3 个 owner 变化，pointer hash 前后不变；尚未 apply。
...
- 未解决限制：三套未参与调参的 ATX/ITX/NAS 实物 holdout 缺失；N6 phase A 的真实 official/current-price closure、phase B 与跨产品清单尚未形成生产证据。离线盘点确认仓库内只有 N6 与 W680M 的正式手册；i5-14500、980 PRO 和旧款 SSR-850FX 只有 catalog 属性、URL 或展示图，不能提升为 official fact。真实 runtime facts/price apply、目录所有权修复、远程推送、生产部署与部署后验证未执行。
