...
- `npm run agent:secret-scan`: 1057 files scanned, zero findings. The same 1057-file result passes through the no-child-process filesystem fallback, which honors the root `.gitignore`, skips symlinks, and keeps ignored local environment files out of output.
...
- Permission repair now has a separate password-free, read-only inspection before backup preparation. It reports only aggregate mode counts, writes neither backup nor repair plan, and requires the existing verified-backup plus second-confirmation flow before apply. Against the persistent runtime it returned `blocked_unreadable`; the active-pointer SHA-256 stayed `0f92903f37e44a7e9f856053093c9f5288c015ab83da3d01a160e4d69d04cf24`, confirming that administrative ownership/read access must be restored before the governed backup step can begin.
- `npm run runtime:bootstrap-access -- --runtime-root runtime --uid 1000 --gid 1000` now covers that narrow pre-backup access deadlock without broad recursive shell commands. Its real-runtime default dry-run returned plan hash `6cc2044a1336fadb23cdf36caa6fe024cd7caaf2b78ede7f6e050b2b776d6348`, generation 1, exactly three frozen targets and three required owner changes; it wrote no artifact and left the active-pointer hash unchanged. Apply was not run. Any later administrator apply must use an explicitly persisted reviewed plan, exact expected plan hash, a private rollback destination outside runtime and the exact confirmation token; writes are descriptor-bound with `O_NOFOLLOW`. Rollback requires the original plan, manifest and both expected hashes before the post-access encrypted-backup/Doctor sequence.
