...
1. 当前 `BuildConfig` 只是让界面可渲染的内部脚手架，不是用户选择、推荐结果或可信基线。不得以“当前已经有这些零件”为由跳过需求收集。
...
7. 官网搜索用于核对现有候选身份和缺失字段。`same-family`、`conflict`、`insufficient-evidence` 或未治理域名都不能成为初始化配置中的新 SKU。
